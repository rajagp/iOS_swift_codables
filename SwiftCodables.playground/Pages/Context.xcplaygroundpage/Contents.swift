...
In this rather contrived example, we will look at a different way of handling associative enum case that we discussed in an earlier chapter.
 If possible, instead of inferring the type at the type of encoding/decoding, you can use some external context information provided as part of the JSON to determine how to encode/decode the data*/
import Foundation
...
/*:
 ...
 In the earlier version of the problem, we detected whether or not the `image` is of type URL or not by
invoking the isValidURLString() method when encoding/decoding. In this version, we will detect the type and send that as part of context info to the encoder/decoder
...
extension UserProfile {
    enum ImageType :Codable {
        ...
        public init(from decoder: Decoder) throws {
            ...
            let key = CodingUserInfoKey.init(rawValue:"imageType")!
            
            let context = decoder.userInfo[key] as? String
            switch context {
                case "url"?:
                    if let imageStr = try? container.decode(String.self) {
                        self = ImageType.url(URL.init(string: imageStr)!)
                    }
                    else {
                        self = ImageType.base64String("" )
                    }
                default:
                    if let imageStr = try? container.decode(String.self) {
                       self = ImageType.base64String(imageStr )
                    }
                    else {
                        self = ImageType.base64String("" )
                    }
...
/*:
 Encoding and Decoding Logic - When image is specified as a URL. Assume for sake of the example that we have some way to identify that we are getting a URL for the image type
...
if let jsonData1 = userProfile1.data(using: .utf8) {
    ...
    do {
        ...
        decoder.userInfo = [CodingUserInfoKey.init(rawValue:"imageType")!:"url"]
        ...
        encoder.userInfo = [CodingUserInfoKey.init(rawValue:"imageType")!:"url"]
...
/*:
 Encoding and Decoding Logic - When image is specified as a base64 encoded string.Assume for sake of the example that we have some way to identify that we are getting a base64 encoded string for the image type
...
if let jsonData2 = userProfile2.data(using: .utf8) {
    ...
    do {
        ...
        // Instance of JSON Decoder
        let decoder = JSONDecoder.init()
        decoder.userInfo = [CodingUserInfoKey.init(rawValue:"imageType")!:"base64"]
        ...
        // Now encode this back
        let encoder = JSONEncoder.init()
         encoder.userInfo = [CodingUserInfoKey.init(rawValue:"imageType")!:"base64"]
        encoder.outputFormatting = .prettyPrinted
        let myUserData = try encoder.encode(myUser)
